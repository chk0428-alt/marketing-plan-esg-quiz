-- 애터미 학습 퀴즈 앱 — 5단계 기념 컬렉션: "새 카드 획득" 미확인 표시 마이그레이션
--
-- 전제: supabase-schema-collections.sql, supabase-schema-collections-streaks.sql이
--       이미 실행되어 있어야 합니다.
-- 사용 방법: Supabase 대시보드 > SQL Editor > New query 에 아래 전체를 붙여넣고 실행하세요.
--
-- 카드를 발급받은 직후의 결과 화면 안내(🎉 새 컬렉션 획득)와는 별개로, "내 컬렉션" 목록에서도
-- 아직 열어보지 않은 카드를 계속 표시해뒀다가, 실제로 클릭(리빌)했을 때만 지우기 위한 상태다.

-- 1) 카드별 확인 여부. 새로 발급될 때는 항상 false(미확인)로 시작한다.
alter table public.quiz_badges
  add column if not exists seen boolean not null default false;

-- 2) 사용자가 "내 컬렉션"에서 카드를 클릭(리빌)했을 때 호출 — 본인 카드만 갱신 가능.
create or replace function public.mark_badge_seen(p_code text)
returns void
language sql
security definer
set search_path = public
as $$
  update public.quiz_badges set seen = true where user_id = auth.uid() and badge_code = p_code;
$$;
grant execute on function public.mark_badge_seen(text) to authenticated;

-- 3) evaluate_and_award_badges() 재정의 -- 반환 타입에 seen 컬럼이 추가돼 기존 함수와
--    시그니처(반환 타입)가 달라지므로 먼저 지운 뒤 다시 만든다.
